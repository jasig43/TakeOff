...
`/` landing · `/register` · `/verify-otp` · `/login` · `/admin/login` · `/driver/dashboard` (applicant only) · `/admin/dashboard` (admin only)
...
src/
  ...
  components/   layout/  landing/  auth/  ui/
  context/      AuthContext, ThemeContext, ToastContext (+ *.ts context objects)
  hooks/        useCountUp, usePasswordValidation, useCountdown, useApiResource, …
...
Vitest + Testing Library: password rules (every special character, boundaries, strength levels), form validators, `OtpInput` (numeric only, auto-advance, backspace, paste, arrows), `ProtectedRoute` role matrix and expired sessions, `useCountUp` (including reduced motion).
