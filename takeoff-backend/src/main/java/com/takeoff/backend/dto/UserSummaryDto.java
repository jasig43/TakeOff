...
/**
 * Safe subset of a user. Never contains the password hash. {@code mustChangePassword} tells the client to send the
 * person to the "choose a new password" screen before anything else.
 */
public record UserSummaryDto(Long id, String fullName, String email, String phoneNumber, Role role,
		boolean phoneVerified, boolean mustChangePassword) {

	public static UserSummaryDto from(User user) {
		return new UserSummaryDto(user.getId(), user.getFullName(), user.getEmail(), user.getPhoneNumber(),
				user.getRole(), user.isPhoneVerified(), user.isMustChangePassword());
	}
}
